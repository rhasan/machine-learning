...
def k_medoids(X, initial_center_idxs, max_iters, distance_matrix):
    m = np.size(X, 0)
...
def initial_random_centers_cost_minimization(X, K, distance_matrix, random_shuffel_max_iters, kmedoids_max_iters):
    min_cost = np.inf
    min_center_idxs = None
    for i in xrange(random_shuffel_max_iters):
        (initial_centers, initial_center_idxs) = initial_random_centers(X, K)
        (center_idxs, idx) = k_medoids(X, initial_center_idxs, kmedoids_max_iters, distance_matrix)
...
def elbow_method_choose_k_with_random_init_cost_minimization(X, max_K, distance_matrix, random_shuffel_max_iters,
                                                             kmedoids_max_iters):
    cost_array = np.zeros(max_K, dtype=float)
    for K in xrange(1, max_K + 1):
        (init_center_idxs, cost) = initial_random_centers_cost_minimization(X, K, distance_matrix,
                                                                            random_shuffel_max_iters,
                                                                            kmedoids_max_iters)
        (center_idxs, idx) = k_medoids(X, init_center_idxs, kmedoids_max_iters, distance_matrix)
        total_cost = model_cost(X, idx, center_idxs, distance_matrix)
...
# elbow_method_choose_k_with_random_init_cost_minimization(X,K,distance_matrix,random_shuffel_max_iters,kmedoids_max_iters)

new_K = 10

(min_center_idxs, min_cost) = initial_random_centers_cost_minimization(X, new_K, distance_matrix,
                                                                       random_shuffel_max_iters, kmedoids_max_iters)
print "min model cost: ", min_cost

(center_idxs, idx) = k_medoids(X, min_center_idxs, kmedoids_max_iters, distance_matrix)
print_clusters(X, idx, center_idxs)
total_cost = model_cost(X, idx, center_idxs, distance_matrix)
print "model cost: ", total_cost
